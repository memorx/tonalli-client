...
const TAB_KEYS = [
  ...
  { key: 'revisionRequested', value: 'REVISION_REQUESTED' },
] as const
    ...
    return (
      <div className="space-y-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{ta('count', { count: approvals.length })}</p>
        </div>
        ...
        {approvals.length > 0 ? (
              ...
              >
                <div className="flex items-center gap-3">
                  {approval.file?.thumbnailUrl ? (
                    ...
                    <img src={approval.file.thumbnailUrl} alt="" className="h-12 w-12 rounded object-cover" />
                  ) : (
          ...
          <div className="rounded-xl border border-border bg-card p-12 text-center">
            ...
            <p className="mt-3 text-sm text-muted-foreground">{ta('empty')}</p>
          </div>
        )}
      </div>
    )
