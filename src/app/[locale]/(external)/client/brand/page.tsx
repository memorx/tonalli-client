      ...
      <div className="space-y-8">
        ...
        {!hasAssets && (
          <div className="rounded-xl border border-border bg-card p-12 text-center">
            ...
            <p className="mt-3 text-sm text-muted-foreground">{tb('empty')}</p>
          </div>
        )}
        ...
        {logos.length > 0 && (
          <section className="space-y-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {logos.map((logo) => (
                <div key={logo.id} className="rounded-xl border border-border bg-card p-6 flex flex-col items-center gap-3">
                  {logo.value.startsWith('http') || logo.value.startsWith('/') ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={logo.fileUrl ?? logo.value} alt={logo.name} className="h-20 w-auto object-contain" />
                  ) : (
        ...
        {typography.length > 0 && (
          <section className="space-y-4">
            <h2 className="flex items-center gap-2 text-sm font-semibold uppercase tracking-wider text-muted-foreground">
              ...
              {tb('sections.typography')}
            </h2>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {typography.map((font) => (
                <div key={font.id} className="rounded-xl border border-border bg-card p-6">
                  <p className="text-3xl font-light text-card-foreground" style={{ fontFamily: font.value }}>
                    Aa Bb Cc 123
                  </p>
        ...
        {guidelines.length > 0 && (
          <section className="space-y-4">
            <h2 className="flex items-center gap-2 text-sm font-semibold uppercase tracking-wider text-muted-foreground">
              ...
              {tb('sections.guidelines')}
            </h2>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {guidelines.map((guide) => (
                <div key={guide.id} className="rounded-xl border border-border bg-card p-5">
